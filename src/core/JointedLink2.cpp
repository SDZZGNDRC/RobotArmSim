...
JointedLink2::JointedLink2(DhParam params, float joint_rotation, int link_number) :
...
void JointedLink2::update(float dt, Affine3f current_world_transform)
{
...
void JointedLink2::updateLinkMatrix(float rotationAngle)
{
	link_matrix_ = this->evalLinkMatrix(rotationAngle);
}

void JointedLink2::updateMesh()
{
...
void JointedLink2::recreateMesh()
{
	mesh_ = JointedLinkMesh2(params_.d, params_.a, link_number_);
}

Affine3f JointedLink2::evalLinkMatrix(float rotationAngle) const
{
...
std::vector<Vertex> JointedLink2::getMeshVertices() const
{
